...
func (r *Runtime) registerConsolidatedTools(s *mcpserver.MCPServer) {
	...
	r.addTool(s, actionToolWithAnnotation("change_manage", "准备或查看 Changeset，不直接修改工作区文件。使用 diff 和 history 审阅；应用或回滚统一使用 change_execute，不要用 command_execute 处理补丁。", map[string]any{
		...
		"history": {Properties: map[string]any{"limit": numberSchema("历史记录数量"), "known_history_digest": stringSchema("上一次 history 返回的 history_digest；相同则返回 not_modified")}},
		"discard": {Properties: map[string]any{"changeset_id": stringSchema("待丢弃的 draft Changeset ID")}, Required: []string{"changeset_id"}},
	...
	r.addTool(s, actionTool("task_manage", "管理 command_execute 启动的运行中命令和项目 Task。使用准确的 remote_session_id 与 task_id 执行 attach、status、logs、stop 或 stdin；本工具不启动新命令、不修改文件。Task ID 不存在时先 list，不要重试旧 ID。", map[string]any{
		...
		"list":        {Properties: map[string]any{"limit": numberSchema("Task 数量"), "known_task_digest": stringSchema("上一次 Task 列表返回的 task_list_digest；相同则返回 not_modified")}},
...
func (r *Runtime) registerConsolidatedToolsV2(s *mcpserver.MCPServer) {
	...
	r.addTool(s, publicTool("change_prepare", "校验文件操作并生成 Changeset 草稿；默认不应用，明确要求且策略允许时可设置 apply=true 在同一次调用中应用。", map[string]any{"remote_session_id": remoteSession, "summary": stringSchema("变更摘要"), "operations": changeItems, "idempotency_key": stringSchema("业务幂等键"), "apply": booleanSchema("是否在准备成功后立即应用；默认 false，需要用户确认时仍返回 confirmation_token"), "format": booleanSchema("应用前格式化变更文件"), "verify": arraySchema(map[string]any{"type": "string"}, "应用后验证步骤"), "purpose": stringSchema("本次调用的目的；必须由用户明确提供")}, []string{"remote_session_id", "operations", "purpose"}, mutatingToolAnnotation), r.toolChangePreparePublic)
